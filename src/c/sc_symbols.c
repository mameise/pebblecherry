/*
 * sc_symbols.c — gezeichnete Super-Cherry-Symbole.
 * Bewusst klar lesbare, retro-anmutende Vektorgrafik in jeder Zelle.
 */
...
/* --- einfache Frucht: gefuellter Kreis mit Outline, Blatt, Glanz ---- */
static void draw_round_fruit(GContext *ctx, GRect cell, GColor body) {
    GPoint c = center_of(cell);
    int rad = (cell.size.w < cell.size.h ? cell.size.w : cell.size.h)/2 - 3;
    if (rad < 4) rad = 4;
    ...
    graphics_draw_line(ctx, GPoint(c.x, c.y - rad), GPoint(c.x + 2, c.y - rad - 4));
    graphics_context_set_fill_color(ctx, C_LEAF);
    graphics_fill_circle(ctx, GPoint(c.x + 4, c.y - rad - 3), 2);
    /* Glanzpunkt links oben */
    fill_circle(ctx, C_BAR, GPoint(c.x - rad/2, c.y - rad/2), 2);
}

/* --- Birne: eifoermig (zwei gestapelte Kreise) ---------------------- */
static void draw_pear(GContext *ctx, GRect cell, GColor body) {
    GPoint c = center_of(cell);
    int rad = cell.size.w/2 - 5; if (rad < 4) rad = 4;
    ...
    graphics_context_set_fill_color(ctx, C_LEAF);
    graphics_fill_circle(ctx, GPoint(c.x + 3, c.y - rad - 1), 2);
    fill_circle(ctx, C_BAR, GPoint(bottom.x - rad/2, bottom.y), 2);
}

/* --- Kirsche: zwei rote Beeren an Stielen + Blatt ------------------- */
static void draw_cherry(GContext *ctx, GRect cell) {
    GPoint c = center_of(cell);
    int rad = cell.size.w/5; if (rad < 4) rad = 4;
    ...
    /* Glanzpunkt */
    fill_circle(ctx, C_BAR, GPoint(l.x - rad/3, l.y - rad/3), 1);
    fill_circle(ctx, C_BAR, GPoint(r.x - rad/3, r.y - rad/3), 1);
}

/* --- Glocke --------------------------------------------------------- */
static void draw_bell(GContext *ctx, GRect cell) {
    GPoint c = center_of(cell);
    int w = cell.size.w/2 - 3, h = cell.size.h/2 - 3;
    if (w < 5) w = 5;
    if (h < 5) h = 5;
    graphics_context_set_fill_color(ctx, C_BELL);
    /* Glockenkoerper: Trapez aus Rechteck + Kreissegment-Naeherung */
...
/* --- BAR: blaues Schild mit weisser Schrift "BAR" ------------------- */
static void draw_bar(GContext *ctx, GRect cell) {
    GPoint c = center_of(cell);
    int w = cell.size.w - 6, h = cell.size.h/2;
    if (w < 10) w = 10;
    if (h < 8) h = 8;
    ...
    graphics_draw_text(ctx, "BAR",
        ...
        GRect(plate.origin.x, plate.origin.y - 3, plate.size.w, plate.size.h + 4),
        GTextOverflowModeFill, GTextAlignmentCenter, NULL);
}

/* --- Stern (echter 5-zackiger gefuellter Stern via GPath) ----------- */
static void draw_star(GContext *ctx, GRect cell) {
    GPoint c = center_of(cell);
    int R = (cell.size.w < cell.size.h ? cell.size.w : cell.size.h)/2 - 2;
    if (R < 6) R = 6;
    int r = R / 2;  /* Innenradius */

    /* 10 Punkte (abwechselnd aussen/innen), feste Grad-Tabelle.
     * cos/sin als skalierte Ganzzahlen (×1000) zur Vermeidung von float. */
...
/* --- Trauben (Cluster aus kleinen Kreisen) -------------------------- */
static void draw_grape(GContext *ctx, GRect cell) {
    GPoint c = center_of(cell);
    int r = cell.size.w/8; if (r < 2) r = 2;
...
/* --- Melone (Halbkreis mit roter Innenseite) ------------------------ */
static void draw_melon(GContext *ctx, GRect cell) {
    GPoint c = center_of(cell);
    int rad = cell.size.w/2 - 4; if (rad < 5) rad = 5;
